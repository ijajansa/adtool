...
class ContinueMetaVideoPublication implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        return 'meta-video-'.$this->campaignId;
    }
    ...
    public function handle(MetaPublishingService $publishing): void
    {
        ...
        if ($attempt->error_code === 'VIDEO_PROCESSING' && $attempt->retryable) {
            ...
            self::dispatch($attempt->id, $this->campaignId)->delay(now()->addSeconds($delays[min($poll - 1, count($delays) - 1)]));
        }
    }
}
